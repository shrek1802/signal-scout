...
public class MainActivity extends Activity {
    ...
    String[] candidateBases = new String[]{
        ...
        "http://tplinkmodem.net", "http://tplinkwifi.net",
...
#home::before{
  ...
  pointer-events:none;
  background:
    linear-gradient(to bottom, rgba(0,4,12,.62) 0%, rgba(0,4,12,.28) 23%, rgba(0,4,12,0) 48%),
    linear-gradient(to bottom, rgba(0,0,0,0) 67%, rgba(0,4,12,.30) 78%, rgba(0,4,12,.82) 100%);
}
.cleanHomeHeader{
  position:absolute;
  left:0;
...
.overlayBtn{
  ...
  display:grid!important;
  grid-template-columns:70px 1fr 46px!important;
  align-items:center!important;
  justify-items:center!important;
...
.overlayBegin,.overlaySetup{
  overflow:hidden!important;
}
.homeOverlayFooter{
  bottom:4.35%!important;
}
