...
public class MainActivity extends Activity {
    ...
    void buildUi() {
        ...
        root.setBackgroundColor(Color.rgb(5,18,31));
        scroll.addView(root);

        ImageView hero = new ImageView(this);
        hero.setImageResource(getResources().getIdentifier("signal_scout_hero", "drawable", getPackageName()));
        hero.setAdjustViewBounds(true);
        hero.setScaleType(ImageView.ScaleType.CENTER_CROP);
        root.addView(hero, new LinearLayout.LayoutParams(-1, 520));

        TextView title = txt("Signal Scout v0.2.0", 28, Color.WHITE);
        title.setGravity(Gravity.CENTER);
        title.setTypeface(null, 1);
        root.addView(title);

        TextView subtitle = txt("Professional LTE & 5G Installation Assistant", 15, Color.rgb(0,180,255));
        ...
        Button loginBtn = btn("LOGIN TO ROUTER");
        root.addView(loginBtn);

        LinearLayout buttons = new LinearLayout(this);
        buttons.setOrientation(LinearLayout.HORIZONTAL);
        root.addView(buttons);
        Button startBtn = btn("START");
        Button stopBtn = btn("STOP");
        Button resetBtn = btn("RESET");
        ...
        Button testBtn = btn("TEST READ ONCE / SHOW RAW REPLY");
        root.addView(testBtn);

        Switch beepSwitch = new Switch(this);
        ...
        startBtn.setOnClickListener(v -> { running=true; bestSinr=-999; status.setText("Running..."); handler.removeCallbacks(poller); handler.post(poller); });
        stopBtn.setOnClickListener(v -> { running=false; handler.removeCallbacks(poller); status.setText("Stopped"); });
        resetBtn.setOnClickListener(v -> { bestSinr=-999; bestView.setText("Best: --"); });
    ...
    EditText edit(String text, String hint, boolean password) {
        EditText e = new EditText(this); e.setText(text); e.setHint(hint); e.setTextColor(Color.WHITE);
    ...
    TextView metric(String s) { TextView v = txt(s, 22, Color.WHITE); v.setGravity(Gravity.CENTER); v.setPadding(8,18,8,18); return v; }
    TextView txt(String s, int size, int color) { TextView v = new TextView(this); v.setText(s); v.setTextSize(size); v.setTextColor(color); return v; }
    String base() { String b = routerUrl.getText().toString().trim(); if (b.endsWith("/")) b=b.substring(0,b.length()-1); return b; }
    ...
    void login() {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                runOnUiThread(() -> { rawView.setText(finalDebug); status.setText(ok ? "Logged in OK - press Test or Start" : "Login not OK - see raw"); });
            } catch(Exception e) {
                final String finalDebug = debug + "\nEXCEPTION:\n" + e.toString();
                runOnUiThread(() -> { status.setText("Login failed: " + e.getClass().getSimpleName()); rawView.setText(finalDebug); });
            }
        }).start();
    }
    ...
    void readSignal() {
        ...
        new Thread(() -> {
            try { String xml = httpGet("/api/device/signal"); runOnUiThread(() -> updateFromXml(xml)); }
            catch (Exception e) { runOnUiThread(() -> { status.setText("Read failed: "+e.getClass().getSimpleName()); rawView.setText(e.toString()); }); }
        }).start();
    }
    ...
    String httpGet(String path) throws Exception {
        ...
        con.setConnectTimeout(5000); con.setReadTimeout(7000); con.setRequestMethod("GET");
    ...
    HttpResult httpPost(String path, String body, String token) throws Exception {
        URL url = new URL(base()+path);
        HttpURLConnection con = (HttpURLConnection) url.openConnection();
        con.setConnectTimeout(5000); con.setReadTimeout(7000); con.setRequestMethod("POST"); con.setDoOutput(true);
    ...
    String headerToken(HttpResult res) {
        for (String k: res.headers.keySet()) if (k!=null && k.equalsIgnoreCase("__RequestVerificationToken")) {
            List<String> vals=res.headers.get(k); if(vals!=null && vals.size()>0) return vals.get(0);
        }
        return "";
    }

    void updateFromXml(String xml) {
        rawView.setText("HTTP 200\n\n"+xml);
        String sinr=pick(xml,"sinr","snr"), rsrp=pick(xml,"rsrp"), rsrq=pick(xml,"rsrq"), rssi=pick(xml,"rssi"), band=pick(xml,"band");
        String pci=pick(xml,"pci"), earfcn=pick(xml,"earfcn"), cell=pick(xml,"cell_id"), enodeb=pick(xml,"enodeb_id");
        ...
        sinrView.setText(sinr.length()>0?sinr:"-- dB"); sinrView.setTextColor(sinrColor(sinrNum));
        rsrpView.setText("RSRP\n"+empty(rsrp)); rsrpView.setTextColor(rsrpColor(num(rsrp)));
        rsrqView.setText("RSRQ\n"+empty(rsrq)); rsrqView.setTextColor(rsrqColor(num(rsrq)));
        rssiView.setText("RSSI\n"+empty(rssi)); bandView.setText("Band\n"+(band.length()>0?"B"+band:"--"));
        cellView.setText("PCI: "+empty(pci)+"\nEARFCN: "+empty(earfcn)+"\nCell ID: "+empty(cell)+"\neNodeB: "+empty(enodeb));
        if (sinrNum>-900) { if (sinrNum>bestSinr) { bestSinr=sinrNum; bestView.setText("Best: "+bestSinr+" dB"); vibrate(); } beepForSinr(sinrNum); }
    }

    void beepForSinr(double sinr) { int d=40; if(sinr>=20)d=130; else if(sinr>=13)d=90; else if(sinr>=5)d=60; tone.startTone(ToneGenerator.TONE_PROP_BEEP,d); }
    ...
    String pick(String xml, String... tags) { for(String tag:tags){ Matcher m=Pattern.compile("<"+tag+">(.*?)</"+tag+">",Pattern.CASE_INSENSITIVE|Pattern.DOTALL).matcher(xml); if(m.find())return m.group(1).trim(); } return ""; }
    ...
    int sinrColor(double v) { if(v>=20)return Color.rgb(60,255,120); if(v>=13)return Color.rgb(200,255,90); if(v>=5)return Color.rgb(255,185,70); return Color.rgb(255,90,90); }
    int rsrpColor(double v) { if(v>=-85)return Color.rgb(60,255,120); if(v>=-95)return Color.rgb(200,255,90); if(v>=-105)return Color.rgb(255,185,70); return Color.rgb(255,90,90); }
    int rsrqColor(double v) { if(v>=-10)return Color.rgb(60,255,120); if(v>=-13)return Color.rgb(200,255,90); if(v>=-15)return Color.rgb(255,185,70); return Color.rgb(255,90,90); }
    void trustAllHttps() { try { TrustManager[] t=new TrustManager[]{new X509TrustManager(){ public X509Certificate[] getAcceptedIssuers(){return new X509Certificate[0];} public void checkClientTrusted(X509Certificate[] c,String a){} public void checkServerTrusted(X509Certificate[] c,String a){} }}; SSLContext sc=SSLContext.getInstance("TLS"); sc.init(null,t,new SecureRandom()); HttpsURLConnection.setDefaultSSLSocketFactory(sc.getSocketFactory()); HttpsURLConnection.setDefaultHostnameVerifier((h,s)->true); } catch(Exception ignored){} }
    static class HttpResult { int code; String body; Map<String,List<String>> headers; HttpResult(int c,String b,Map<String,List<String>> h){code=c;body=b;headers=h;} }
}
